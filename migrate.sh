...
migrate() {
  ## packages
  SEARCH_PACKAGES=$(cat ${OLD_ENV_FILE} | grep -E ${PACKAGES} | sed 's/"//g' | sed 's/PACKAGES=//g')
  ...
  ## repositories
  SEARCH_REPOSITORIES=$(cat ${ENV_FILE} | grep -E "azure-cli|docker-ce|nomad|consul" | sed 's/-\ //g')
  ...
  ## binaries
  SEARCH_BINARIES=$(cat ${OLD_ENV_FILE} | grep -E ${BINARIES})
  GITHUB_COUNT=0
  ...
  ## requirements
  SEARCH_REQUIREMENTS=$(cat ${OLD_ENV_FILE} | grep -E ${REQUIREMENTS})
  if [ "${SEARCH_REQUIREMENTS}" != "" ]; then
    ...
    do
      ...
      echo "  ${REQ}:" >> ${ENV_FILE}
      if [ $REQ = "pip" ]; then
        for REQ_CONTENT in $(cat ${ENV_DIR}/${REQ_LIST} | sed 's/==/=/g')
        do
          echo "    - ${REQ_CONTENT}" >> ${ENV_FILE}
        done
        elif [ $REQ = "ansible" ]; then
        for REQ_CONTENT in $(cat ${ENV_DIR}/${REQ_LIST} | sed 's/^#.*//g' | tr -d '-' | tr -d ' ' | sed 's/name:/ /g' | sed 's/version:/=/g' | sed '/^.*:/ s/./#&/' | tr -d '\n' | tr '#' '\n')
        do
  ...
  ## extensions
  SEARCH_EXTENSIONS=$(cat ${OLD_ENV_FILE} | grep -E ${EXTENSIONS})
